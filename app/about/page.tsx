...
export default function About() {
  ...
  const dict = {
    ...
    ko: {
      originDesc1: <>챔피언 프레싱은 현대 바이닐(LP) 대량 생산 과정에서 흔히 발생하는 음질적 타협에 대한 깊은 아쉬움에서 출발했습니다.<br />우리는 레코드를 프레싱하는 과정이 단순한 '제조'가 아니라, 음악을 완성하는 마스터링의 가장 핵심적이고 최종적인 단계라고 믿습니다.</>,
      originDesc2: <>대한민국 남양주에 위치한 우리의 팩토리는 설립 초기부터 생산량(Volume)보다 사운드의 해상력(Fidelity)을 최우선으로 고려하여 설계되었습니다.<br />수십 년간 이어져 온 아날로그 전통의 기술력과 최상의 컨디션으로 관리되는 최신 프레싱 설비가 결합된 결과물입니다.</>,
      standardDesc1: <>챔피언 프레싱은 엔지니어이자 전문 리스너들로 구성된 소수 정예 팀입니다.<br />운영 규모를 무리하게 확장하지 않는 대신, 테스트 프레싱부터 최종 패키징에 이르는 모든 단계에서 무결점의 품질 관리를 유지합니다.</>,
      standardDesc2: <>데뷔 EP를 준비하는 독립 아티스트부터 클래식 명반을 재발매하는 대형 레거시 레이블까지, 우리가 레코드를 대하는 태도는 동일합니다.<br />타협 없는 정밀도, 투명한 커뮤니케이션, 그리고 바이닐 그루브에 새겨지는 음악에 대한 깊은 존중입니다.</>,
      readyDesc: <>여러분의 다음 프로젝트에 대한 이야기를 들려주세요.<br />치열하게 고민한 사운드를 가장 완벽한 물리적 매체로 완성해 드립니다.</>,
      makeQuoteBtn: "Make a Quote",
      backToTopBtn: "Back to Top"
    }
  };
