...
export default function Pressing() {
  ...
  const dict = {
    ...
    ko: {
      heroDesc: <>바이닐(LP) 생산은 단순한 복제가 아닙니다. 소리를 물리적인 매체로 변환하는 매우 섬세하고 예민한 과정입니다.<br />챔피언 프레싱의 전문 컨설턴트들은 마스터링 스튜디오부터 최종 납품에 이르기까지 아티스트와 긴밀하게 협력하여, 당신이 그리는 사운드의 질감이 레코드 그루브 안에 온전히 담길 수 있도록 타협 없이 검수합니다.</>,
      p1Desc: <>디지털 또는 아날로그 마스터 음원이 물리적인 그루브로 처음 치환되는 가장 결정적인 단계입니다.<br />우리는 전문 커팅 엔지니어와의 긴밀한 소통을 통해, 바이닐 매체 단위에서 허용 가능한 라우드니스(음압), 다이내믹스, 그리고 주파수 밸런스가 완벽하게 최적화되도록 조율합니다.</>,
      p2Desc: <>커팅이 완료된 래커 마스터(Lacquer Master) 표면에 은과 니켈을 전기도금하여 실제 프레싱에 사용될 메탈 스탬퍼(Metal Stampers)를 생성합니다.<br />미세한 먼지나 결함조차 표면 노이즈로 직결되기 때문에, 모든 과정은 엄격하게 통제된 무결점 환경에서 진행됩니다.</>,
      p3Desc: <>스탬퍼가 프레스기에 장착되고, 극도의 열과 압력을 가해 PVC 펠릿을 형태화합니다.<br />본 생산에 앞서 테스트 프레싱(TP) 공정을 필수적으로 거치며, 전문 컨설턴트와 아티스트가 함께 청취하여 팝 노이즈, 디스토션, 다이내믹 무결성 등을 철저하게 검증합니다.</>,
      p4Desc: <>피지컬 매체의 시각적 완성도는 사운드만큼이나 중요합니다.<br />재킷 스펙의 컬러 정확도, 지류(Paper Stock)의 질감, 인쇄 마감 상태를 집요하게 인스펙션합니다.<br />이 모든 과정을 거친 레코드는 가장 안전하게 패키징되어 당신의 손으로 직접 배송됩니다.</>,
