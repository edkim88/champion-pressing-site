          ...
          <div className="hidden lg:flex items-center h-[48px] gap-10 lg:gap-14 text-sm uppercase tracking-[0.12em]">
            {navLinks.map((link) => (
              <Link 
                key={link.href} 
                href={link.href} 
                className={`hover:opacity-55 transition ${link.href === '/quality' ? 'text-black font-semibold' : ''}`}
              >
                {link.label}
              </Link>
            ))}
          ...
          >
            {navLinks.map((link) => (
              <Link 
                ...
                className={`text-2xl md:text-3xl uppercase tracking-[0.15em] hover:opacity-50 transition ${link.href === '/quality' ? 'border-b border-black' : ''}`}
              >
                {link.label}
              </Link>
            ))}
        ...
        <div className="max-w-7xl mx-auto">
          <div className="grid md:grid-cols-12 gap-10 items-stretch mb-12">
            ...
            <div className="md:col-span-4 flex flex-col justify-center py-6 md:py-10">
              ...
              <h2 className="text-2xl md:text-4xl font-semibold tracking-[-0.03em] leading-tight mb-6 uppercase">
                Finest Monitoring,<br />Where the Master is Made.
              </h2>
              <p className="text-sm md:text-base leading-relaxed text-black/65">
                We operate within a world-class monitoring environment anchored by the Genelec 8381A flagship system. This allows us to perform vinyl mastering and test-pressing evaluations with unparalleled resolution.
              </p>
            </div>
          </div>
        </div>
          ...
          <div className="flex flex-col md:flex-row items-stretch justify-between gap-12">
            <div className="flex-1 flex flex-col justify-center py-6 md:py-10 max-w-xl">
              ...
              <p className="text-base md:text-lg leading-relaxed text-black/65">
                From the sonic depth of mastering to the tactile precision of pressing — we see every record through the eyes of the creator, ensuring your artistic intent is never lost in translation.
              </p>
            </div>
      ...
      <section className="px-7 md:px-10 pb-24 md:pb-32">
        <div className="max-w-7xl mx-auto border-t border-black/8 pt-8 md:pt-10 flex flex-col md:flex-row md:items-end md:justify-between gap-8">
          ...
          <div className="flex gap-4 md:gap-6 items-center">
            <Link
              href="/quote"
              className="inline-flex w-[140px] md:w-[180px] h-[48px] md:h-[54px] items-center justify-center border border-black bg-transparent text-black text-[10px] md:text-[11px] font-normal uppercase tracking-[0.22em] leading-none appearance-none hover:bg-black hover:text-[#f5f3ee] transition-all duration-300 whitespace-nowrap"
            >
              Make a Quote
            </Link>
